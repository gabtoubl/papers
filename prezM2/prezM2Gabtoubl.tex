...
\subtitle{$K$-mer et $k$-mer espacés}
...
\begin{frame}[fragile]{Le séquençage ADN}
  ...
  \begin{overprint}
    ...
    \begin{itemize}[<+-| alert@+>]
      \item Lectures courtes (depuis 2005, Illumina, Roche, ...)
      \item Lectures longues (depuis 2010, Oxford Nanopore, Pacific Biosciences)
    \end{itemize}
  \end{overprint}
\end{frame}
...
\begin{frame}[fragile]{$K$-mers espacés à insertion}
  ...
  $\mathbf{(L - k + 1)*4^t}$ $k$-mers espacés à insertion possibles.\medskip\\\pause
...
\section{Implémentation : KmersExpand}
...
\section{Implémentation: Mots Minimaux absents (MAWs)}
...
\section{Implémentation: Plus long sous-mot commun}
